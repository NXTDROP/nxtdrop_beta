<?php
    ...
    if(isset($_GET['offerID'])) {
        ...
        $userID = $_GET['userID'];
        $getInfo = $conn->prepare("SELECT c.offer, o.price, p.model, u.username FROM users u, counterOffer c, products p, offers o WHERE o.offerID = ? AND c.userID = ? AND o.offerID = c.offerID AND c.userID = u.uid AND o.productID = p.productID");
        $getInfo->bind_param('ii', $offerID, $userID);
        $getInfo->execute();
        $getInfo->bind_result($cprice, $iprice, $model, $username);
        $getInfo->fetch();
    }
    else {
        $email = '';
...
<html lang="en">
    <head>
        ...
        <title>You have a counter-offer! Will you take it?</title>
