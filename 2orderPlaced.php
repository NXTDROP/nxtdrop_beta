...
<script>
    fbq('track', 'Purchase');
        ...
        <?php
            ...
            $query = "SELECT p.assetURL, p.model, o.retailPrice, s.cost, t.shippingAddress, s.MM_Carrier, s.MM_TrackingNumber, t.transactionID, t.totalPrice, t.sellerID, t.middlemanID, tb.first_name, tb.last_name, t.status, u.username FROM users u, transactions t, shipping s, thebag tb, holidayOffers o, products p WHERE t.transactionID = '$transactionID' AND t.transactionID = s.transactionID AND t.sellerID = u.uid AND u.uid = tb.uid AND t.itemID = o.productID AND o.productID = p.productID";
            ...
            if($_SESSION['country'] == 'US') {
                ...
                $row['price'] = usdTocad($row['retailPrice'], $db, false);
